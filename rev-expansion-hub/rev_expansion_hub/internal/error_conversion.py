...
from rev_rhsplib import RhspLibError as RhspLibCError, RhspLibErrorCode
...
def _create_error(e: Exception, serial_number: str | None) -> Exception:
    if not isinstance(e, RhspLibCError):
        return e
    ...
    if code == ec.SERIAL_ERROR:
        return GeneralSerialError(serial_number or "no serial number provided")
    if ec.ARG_OUT_OF_RANGE_END <= code <= ec.ARG_OUT_OF_RANGE_START:
        index = -code + int(ec.ARG_OUT_OF_RANGE_START)
